...
def getClassWeights():
    ...
    weight = torch.ones(21, dtype=torch.float32).to(device)

    for input, label in train_loader:
        label = label.reshape(-1).to(device)  # Assuming this is necessary for your label shape
        unique_labels, counts = label.unique(return_counts=True)
        weight[unique_labels] += counts
...
def train():
    ...
    for epoch in range(epochs):
        ...
        train_losses.append(np.mean(ep_loss)) #for graph
        ...
        if current_miou_score > best_iou_score:
            ...
            torch.save(fcn_model.state_dict(), "./models/4c.pth")

    #fcn_model.load_state_dict(torch.load("./models/4c.pth")) 
