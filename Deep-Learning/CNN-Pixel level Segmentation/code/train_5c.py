...
def getClassWeights():
    ...
    weight = torch.ones(21, dtype=torch.float32)
...
fcn_model = FCN_5c(n_class=n_class)
fcn_model.apply(init_weights)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
optimizer = torch.optim.Adam(fcn_model.parameters(), lr=0.0001)
criterion =  nn.CrossEntropyLoss(weight=getClassWeights())
...
def train():
    ...
    for epoch in range(epochs):
        ...
        for iter, (inputs, labels) in enumerate(train_loader):
            ...
            loss_clone = torch.clone(loss).item() #for graph
        ...
        val_losses.append(ep_val_loss) #for graph
        ave_val_iou.append(current_miou_score)
        val_acc.append(ep_acc)

        if current_miou_score > best_iou_score:
            best_iou_score = current_miou_score
            torch.save(fcn_model.state_dict(), "./models/5c.pth")

    fcn_model.load_state_dict(torch.load("./models/5c.pth")) 
